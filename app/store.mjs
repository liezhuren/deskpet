...
//   · 数据量很小：记忆上限 400 条 × 约 200 字节 ≈ 80KB；其余（设置/监视状态/人格）都是几 KB
//   · 只有一个进程在写，不需要并发事务
...
export const SETTING_PATHS = Object.freeze([
  'game.dir', 'game.adapter', 'game.level', 'game.cardId', 'game.processes', 'game.logPatterns', 'game.saveRules',
...
export function redactSettings(s) {
  const out = clone(s)
  if (out?.llm && typeof out.llm.apiKey === 'string' && out.llm.apiKey !== '') {
    const k = out.llm.apiKey
    out.llm.apiKey = k.length <= 8 ? '****' : `${k.slice(0, 4)}****${k.slice(-4)}`
    out.llm.apiKeySet = true
  } else if (out?.llm) {
    out.llm.apiKey = ''
    out.llm.apiKeySet = false
  }
  return out
}
